...
import { SalesKPITargetsEditor } from './SalesKPITargetsEditor';
import {
  Sliders,
  CheckCircle2,
  AlertTriangle,
...
export const SalesSettingsView: React.FC = () => {
  ...
  const [activeMainTab, setActiveMainTab] = useState<'kpi-targets' | 'weights' | 'rewards' | 'currency'>('kpi-targets');
  const [activeProfileTab, setActiveProfileTab] = useState<SalesProfileCode>('PR');
  const [formData, setFormData] = useState<SalesRewardSettings>(JSON.parse(JSON.stringify(salesSettings || DEFAULT_SALES_SETTINGS)));
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Tab Navigation */}
      <div className="flex items-center gap-2 p-1.5 bg-[#f4f7f0] border border-[#e2ebd9] rounded-2xl overflow-x-auto shadow-2xs">
        <button
          type="button"
          onClick={() => setActiveMainTab('kpi-targets')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${
            activeMainTab === 'kpi-targets'
              ? 'bg-[#8cc540] text-[#101010] shadow-xs'
              : 'text-[#555555] hover:text-[#101010] hover:bg-white/60 font-bold'
          }`}
        >
          <Target className="w-4 h-4" />
          🎯 Weekly & Monthly KPI Targets
        </button>
        ...
        >
          ...
          ⚖️ Scoring Weights & Hurdle (50/20/30)
        </button>
        ...
        >
          ...
          🎁 Reward Slabs & Payouts
        </button>

        <button
          type="button"
          onClick={() => setActiveMainTab('currency')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${
            activeMainTab === 'currency'
              ? 'bg-[#8cc540] text-[#101010] shadow-xs'
              : 'text-[#555555] hover:text-[#101010] hover:bg-white/60 font-bold'
          }`}
        >
          <Sliders className="w-4 h-4" />
          ⚙️ Currency & General Settings
        </button>
      </div>

      {/* TAB 1: WEEKLY & MONTHLY KPI TARGETS (NEW FULL-FEATURED EDITOR) */}
      {activeMainTab === 'kpi-targets' && (
        <SalesKPITargetsEditor />
      )}

      {/* TAB 4: CURRENCY & GLOBAL PREFERENCES */}
      {activeMainTab === 'currency' && (
          ...
          <div className="flex items-center justify-between border-b border-[#e2ebd9] pb-4">
            <div>
              ...
              <p className="text-xs text-[#666666]">
                Configure monetary display symbol across all Sales modules, leaderboards, and reward vouchers
              </p>
            </div>
          </div>

          <div className="flex items-center justify-between p-4 rounded-2xl bg-[#f8faf6] border border-[#e2ebd9]">
            <div>
              <span className="text-xs font-black text-[#101010] block">Active Currency Unit</span>
              <span className="text-[11px] text-[#666666]">Choose between USD ($) and INR (₹)</span>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                ...
                className={`px-4 py-2 rounded-xl text-xs font-black border transition-all cursor-pointer ${
                  formData.currencySymbol === '$'
                    ? 'bg-[#8cc540] text-[#101010] border-[#8cc540] shadow-xs'
                    : 'bg-white text-[#555555] border-[#e2ebd9]'
                }`}
              >
                $ USD (Dollars)
              </button>
      ...
      {/* TAB 2 & 3: PROFILE SELECTOR & DETAILED CONFIG */}
      {(activeMainTab === 'weights' || activeMainTab === 'rewards') && (
        <div className="space-y-6">
          {/* Profile Tabs */}
          ...
          <div className="bg-white rounded-3xl border border-[#e2ebd9] p-6 shadow-xs space-y-6">
            ...
            {/* If tab is weights: Show 4 Performance Metric Target & Weight Inputs + Hurdle */}
            {activeMainTab === 'weights' && (
              <>
                  ...
                  <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        min="1"
                        value={currentConfig.targetOrderValue ?? currentConfig.orderValueTarget ?? 100000}
                        onChange={(e) => {
                          handleProfileFieldChange('targetOrderValue', Number(e.target.value));
                          handleProfileFieldChange('orderValueTarget', Number(e.target.value));
                        }}
            ...
            {/* If tab is rewards: Show Reward Slabs & Cash Amounts Table */}
            {activeMainTab === 'rewards' && (
