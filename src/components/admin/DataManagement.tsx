...
export const DataManagement: React.FC<DataManagementProps> = ({ onOpenImportModal }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/95 p-5 rounded-3xl border border-slate-750 shadow-xl">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-2 rounded-xl bg-blue-500/20 text-blue-300 border border-blue-400/40">
          ...
          <p className="text-xs text-slate-300 mt-1">
            Master records for <span className="text-white font-semibold">{selectedMonth} {selectedYear}</span> ({filteredRecords.length} records found)
          </p>
        </div>
          ...
          {isSuperAdmin && (
            <>
              <button
                ...
                className="px-3.5 py-2 rounded-xl text-xs font-bold bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 border border-rose-500/40 flex items-center gap-1.5 transition-colors cursor-pointer"
      ...
      {/* Search & Filters */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-900/90 p-4 rounded-2xl border border-slate-750">
        ...
        <div className="flex items-center gap-2 overflow-x-auto">
          <span className="text-xs text-slate-300 font-bold">Filter Week:</span>
      ...
      <div className="rounded-3xl border border-slate-750 bg-slate-900/90 overflow-hidden shadow-xl">
              ...
              {filteredRecords.map((rec) => (
                <tr key={rec.id} className="hover:bg-slate-800/60 transition-colors">
                  <td className="py-3.5 px-4 font-bold text-white whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <span className="text-white font-bold">{rec.userName}</span>
                      <span
                  ...
                  <td className="py-3.5 px-4 whitespace-nowrap">
                    <span className="px-2 py-0.5 rounded-md bg-slate-800 text-slate-200 border border-slate-700 font-semibold text-[11px]">
                      {rec.weekName} ({rec.month} {rec.year})
                    </span>
                  </td>
                  <td className="py-3.5 px-4 font-bold text-slate-100">{rec.projectClosed}</td>
                  <td className="py-3.5 px-4 whitespace-nowrap">
                  ...
                  <td className="py-3.5 px-4 font-bold text-cyan-300">{rec.upsells}</td>
                  <td className="py-3.5 px-4 font-bold text-amber-300">
                    {rec.clientRating > 0 ? `${rec.clientRating.toFixed(1)} ★` : '0 ★'}
                  </td>
                  <td className="py-3.5 px-4 text-slate-200 font-medium">{rec.followupsCompleted}</td>
                  <td className="py-3.5 px-4 text-slate-200 font-medium">{rec.repeatClients}</td>
                  <td className="py-3.5 px-4 text-slate-300 text-[11px] truncate max-w-[120px] font-medium">
                    {rec.submittedBy}
                  </td>
                      ...
                      <>
                        {deleteConfirmId === rec.id ? (
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-slate-800 hover:bg-rose-900/50 text-slate-300 hover:text-rose-200 border border-slate-700 transition-colors cursor-pointer"
                            title="Delete Record"
                          >
        ...
        {filteredRecords.length === 0 && (
          <div className="text-center py-12 text-slate-300 space-y-2">
            <FileSpreadsheet className="w-10 h-10 mx-auto opacity-40 text-orange-400" />
            <p className="text-sm font-bold text-white">No performance records found for this filter.</p>
            <p className="text-xs text-slate-400">Try changing the week filter or search query above.</p>
          </div>
        )}
      </div>
